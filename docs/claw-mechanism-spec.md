...
바퀴를 돌리는 기어드 모터가 붙는다. 이 부품이 없으면 갠트리는 구동원 없이
움직이는 기구가 된다.

| 부품 | 역할 |
| --- | --- |
| 기어케이스 | 좌측 엔드 플레이트 내측면에 볼트로 고정되고 브리지 봉 3개의 끝단을 감싸는 하우징 |
| 주행 모터 | 기어케이스 아래에 매달린 원통형 모터. 브리지 밑 유일한 빈 공간을 사용한다 |
| 구동 출력 샤프트 | 기어케이스 출력이 플레이트를 관통해 좌측 레일 바퀴 2개를 직접 돌리는 굵은 축 |
| 점검 커버 | 기어케이스 후면의 볼트 고정 판. 내측면은 캐리지 스토퍼이므로 평면을 유지한다 |
| 단자함·케이블 글랜드 | 모터 배선이 빠져나오는 상부 함체와 후면 글랜드 2개 |

좌측 레일 바퀴 2개만 구동륜이고 우측 2개는 유동륜이다. 구동측 샤프트는
`반지름 30mm`, 유동측 스터브 축은 `반지름 16mm`로 렌더링해 어느 쪽이
구동륜인지 형상만으로 구분된다.
...
`x = -1.72m`이므로 모든 인형이 여전히 사정거리 안에 있다. 모터 캔과 단자함은
기어케이스 내측면보다 바깥에 있어 캐리지가 닿는 면은 항상 기어케이스 평면
하나뿐이다.

브리지 질량은 `5.2kg`이고 봉·타이·플레이트·기어케이스·모터에 분배한다.
구동부가 전체의 `26%`를 차지하므로 Z축은 한쪽으로 치우친 실제 부하를
가속한다. 분배 비율의 합은 `tests/drive-axis.test.mjs`에서 검증한다.
...
| 상품 배출구 | 내부 낙하구의 투명 아크릴 벽과 전면 우측의 어두운 `PRIZE OUT` 플랩 |
...
상품 낙하구의 네 아크릴 벽은 예외로, 보이는 벽과 같은 치수의 단순
Cuboid Collider를 사용해 주변 인형이 낙하구로 밀려 들어가는 것을 막고
파지된 인형을 센서 영역으로 안내한다.
...
| 브리지 주행 구동부 | 브리지 강체의 Cuboid·Cylinder Collider | 브리지 질량의 `26%`와 Z축 편심 부하, 캐리지의 좌측 끝단 |
...
- 모든 손가락 경로 Collider에 같은 `clawFriction`을 적용해 금속 손가락
  자체가 파지 마찰을 담당
- 브리지 주행 구동부는 기어케이스 Cuboid와 모터 Cylinder 2개만 두고 점검 커버·단자함·
  글랜드는 렌더 전용. 갠트리 부재끼리는 충돌 그룹이 분리되어 있어 캐리지의 좌측 끝단은
  이 충돌체가 아니라 조인트 리밋이 만든다
