...
- 집게 본체: 동적 RigidBody
- 승강 와이어: 비신축 길이 제약
- 중앙 플런저: 본체 로컬 좌표를 따르는 Kinematic RigidBody
- 로커 링크: 두 원 교점 기구학으로 자세가 결정되는 Kinematic RigidBody
- 손가락: 동일한 기구학을 따르는 Kinematic RigidBody와 복합 콜라이더
- 인형 접촉: 곡선 손가락 콜라이더가 실제로 처리

플런저·로커·손가락의 자세는 9절의 고정 길이 조건으로 계산한다. 계산된
로컬 자세를 매 프레임 집게 본체의 월드 위치와 회전에 합성하므로, 승강이나
수평 이동 중에도 부품이 본체에서 분리되지 않는다. 외관상 핀은 한 축으로만
회전하며 플런저를 관통하지 않는다.
...
- 플런저와 링크의 로컬 자세를 집게 본체의 월드 변환에 합성
...
5. 기구학 자세를 따르는 손가락 복합 콜라이더와 인형 접촉 연결
