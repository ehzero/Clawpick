...
- 구동 입력: 플런저 목표 스트로크 하나와 힘 제한 PD 축력
- 인형 접촉: 동적 곡선 손가락의 단순 복합 콜라이더가 처리

게임 상태는 손가락이나 로커 각도를 직접 변경하지 않고 플런저 직선 조인트의
목표 스트로크만 변경한다. 물리 솔버가 `A-B`, `B-C`의 고정 길이와 각 회전
축의 일치를 동시에 풀어 로커, 이동 힌지와 손가락 자세를 결정한다.
...
- 플런저 축력 제어기만 위치 목표를 가지며 나머지 링크에는 별도 애니메이션을 적용하지 않음
...
| 플런저 속도 | `m/s` | Prismatic Joint 목표 스트로크 변화율 |
| 플런저 최대 축력 | `N` | 플런저에 가하는 구동력의 절댓값 상한 |
| 마찰계수 | `μ` | Rapier Collider의 무차원 Coulomb 마찰계수 |
| 선형·회전 감쇠 | `s⁻¹` | RigidBody 속도 감쇠계수 |
| 인형 질량 | `kg` | 인형 복합 Collider의 총질량 |
| 중력 | `m/s²` | 물리 월드의 Y축 중력 가속도 |

플런저 구동기는 위치 오차와 상대 속도로 PD 요구력을 계산한 뒤
`±plungerMaxForce` 범위로 제한한다. 손가락에 직접 토크를 적용하지 않으며,
실제 힌지 토크는 제한된 플런저 축력과 링크의 순간 기계적 이득으로 결정된다.
닫힘·상승·복귀·개방 단계에서만 구동하며, 대기와 결과 단계에서는 축력을
0으로 만들어 잔류 구동력이 남지 않게 한다.

프리셋 형식은 버전 2를 사용한다. 버전 1의 `closeSpeed`,
`clawStrength`, `swingDamping`은 각각 플런저 속도, 최대 축력, 선형
감쇠계수로 변환한 뒤 허용 범위로 제한한다.
...
| 플런저 | Dynamic RigidBody + Prismatic Joint | 유일한 모터 입력, 중심축 스트로크와 핀 힘 전달 |
...
대기·하강·안정·결과 단계에서는 플런저 축력을 `0 N`으로 둔다. 이때
Prismatic Joint는 스트로크 범위 안에서 자유롭기 때문에 손가락 중력과 세
폐쇄 링크의 미세한 구속 오차가 플런저로 전달될 수 있다. 특히 열린
스토퍼 부근에서 플런저와 손가락이 미세하게 진동하는 현상이 남아 있다.

상시 위치 유지력, 하드 리밋 바깥 여유, 낮은 강성과 과감쇠, 단방향 끝점
예압 방식은 각각 시험했으나 체감 진동이 더 커져 폐기했다. 현재 구현은
이 실험 이전의 단계별 힘 제한 PD 방식으로 되돌린 상태다. 후속 개선은
플런저 상대 변위·속도와 조인트 임펄스를 먼저 계측한 뒤 별도 작업으로
진행한다.
...
- 본체 피벗, 이동 힌지와 플런저 연결 핀은 렌더 치수와 같은 원통으로 구성
- 손가락 끝 패드는 렌더 타원체와 같은 반지름의 Convex Hull로 구성
- 플런저 축·허브·요크와 세 로커 링크는 렌더 치수를 공유하는 단순 충돌체로 구성
...
현재는 플런저 목표 스트로크만 변경하며 세 손가락의 실제 자세는 Rapier
폐쇄 링크 솔버가 결정한다. 플런저 축력은 설정된 최대값으로 제한되므로
인형 접촉 저항에 따라 손가락이 목표 위치에 도달하지 못할 수 있다.

다음 항목은 명목상 결합과 승강 안정성이 검증된 뒤 추가한다.

- 대기 상태의 복귀 스프링·가이드 마찰과 플런저 미세 진동 억제
...
- 와이어 목표 길이
- 플런저 실제 위치·목표 오차·상대 속도
- Prismatic Joint 리밋과 모터 임펄스
- 각 손가락의 목표 각도와 실제 각도
- 각 손가락 모터 토크
- 손가락별 접촉 개수
- 로커 링크와 손가락 상단 길이 오차
...
현재는 1~9단계를 폐쇄형 물리 링크로 구현했다. 렌더와 충돌 형상의 대응,
힘 제한 플런저 구동과 물리 튜닝 단위 정리는 반영됐다. 다만 열린 대기
상태의 플런저 미세 진동은 해결되지 않은 한계로 남아 있으며, 다음 단계에서는
상대 변위·속도와 조인트 임펄스를 계측해 원인을 분리한다.
