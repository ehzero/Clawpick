...
- 우측 투명선: 추가 배선, 안전선 또는 보호 튜브로 추정되지만 사진만으로
  정확한 용도는 확정할 수 없음
...
각 손가락은 하나의 연속된 곡선 부품이며 하나의 힌지를 중심으로 회전한다.
손가락 중간이 한 번 더 꺾이는 2관절 구조가 아니다.

사진에서 손가락 위쪽에 보이는 긴 직선 플레이트는 다음 역할을 하는
고정 브래킷으로 해석하는 것이 타당하다.

- 집게 본체와 손가락 힌지 사이의 거리 확보
- 힌지 핀 양쪽 지지
- 손가락에 전달되는 굽힘 하중을 본체로 전달
- 연결 링크와 손가락 레버가 움직일 공간 확보

따라서 손가락 한 개의 기본 구조는 다음과 같다.

1. 본체에 고정된 브래킷
2. 브래킷 하단의 힌지 핀
3. 힌지를 중심으로 회전하는 곡선 손가락
4. 손가락 루트의 짧은 구동 레버
5. 구동 레버와 중앙 요크를 연결하는 링크
...
| 공통 슬라이더·요크 | 하나의 플런저 운동을 세 링크에 분배 |
| 하부 스파이더 브래킷 | 세 손가락 브래킷을 120도 간격으로 고정 |

### 6.3 손가락 구동부

| 부품 | 역할 |
| --- | --- |
| 고정 브래킷 3개 | 손가락 힌지 위치 고정 |
| 힌지 핀 3개 | 손가락의 단일 회전축 |
| 부싱·와셔·고정링 | 마찰과 축 방향 유격 관리 |
| 연결 링크 3개 | 공통 요크 운동을 각 손가락에 전달 |
| 링크 핀 | 링크와 레버 연결 |
...
- `H`: 손가락 힌지 위치
- `P(q)`: 플런저 또는 요크의 링크 핀 위치
- `L(θ)`: 손가락 루트 레버의 링크 핀 위치
- `r`: 연결 링크 길이
- `θ`: 손가락 회전각

링크 길이는 고정되어 있으므로 다음 조건을 만족해야 한다.

```text
|L(θ) - P(q)| = r
```

이 조건으로 `q`에 대응하는 `θ`를 계산하면 플런저의 직선 운동을 실제
링크 구조와 유사한 비선형 회전으로 변환할 수 있다.
...
브라우저 물리 엔진에서 플런저, 세 링크와 세 손가락을 모두 폐쇄형 물리
조인트로 연결하면 제약 조건이 충돌해 진동하거나 폭발할 수 있다.
...
- 승강 와이어: 비신축 길이 제약
- 손가락: 각각 동적 RigidBody
- 손가락 힌지: Revolute Joint와 각도 제한
- 중앙 플런저: 공유 상태 `q`로 시뮬레이션
- 링크 각도와 위치: 기구학적으로 계산해 렌더링
- 손가락 구동: 목표 각도와 최대 토크가 있는 힌지 모터
- 인형 접촉: 손가락 콜라이더가 실제로 처리

손가락 모터는 목표 각도까지 강제로 순간 이동시키지 않고 다음과 같은
토크 제한 제어를 사용한다.

```text
τ = clamp(
  stiffness × (targetAngle(q) - currentAngle)
...
`maxTorque`가 실제 집게 강도에 해당한다. 인형이 움직임을 막으면 손가락은
목표 각도에 도달하지 못해야 한다.

### 10.2 고정밀 방식

실제 링크 반작용까지 필요할 때만 다음 단계로 확장한다.

- 플런저에 Prismatic Joint 적용
- 플런저에 힘 제한 선형 모터 적용
- 각 링크를 별도 RigidBody로 구성
- 링크 핀에 Revolute 또는 Spherical Joint 적용
- 작은 핀 유격과 컴플라이언스 추가

이 방식은 정확하지만 튜닝 비용과 제약 불안정성이 크게 증가한다.
...
- 고정 브래킷, 힌지 핀, 링크와 중앙 하부 슬라이더가 구분된다.
...
1. 사진 비율을 기준으로 본체, 브래킷과 손가락의 기준 치수 설정
2. 단일 곡선 손가락의 렌더링 메시와 복합 콜라이더 제작
3. 세 손가락의 120도 힌지 배치와 각도 제한 적용
4. 플런저 공유 상태와 링크 역기구학 구현
5. 힘 제한 힌지 모터와 인형 접촉 연결
6. 비신축 승강 와이어와 스위블 흔들림 적용
7. 검은 나선 케이블과 투명 케이블 시각화
