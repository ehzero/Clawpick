...
`ClawBody`의 로컬 좌표를 사용한다. `CurvedFingerCollider`만 물리 월드에
별도 키네마틱 바디로 존재하며, 같은 기구학 자세를 월드 좌표로 변환해
따라간다.
...
- 집게 본체: 동적 RigidBody
- 승강 와이어: 비신축 길이 제약
- 중앙 플런저: 본체 자식으로서 로컬 Y축만 이동하는 시각 그룹
- 로커 링크: 본체 피벗에 고정되어 접선축으로만 회전하는 자식 시각 그룹
- 손가락 외형: 공통 힌지 좌표를 사용하는 본체 자식 시각 그룹
- 손가락 충돌체: 외형과 같은 자세를 따르는 별도 Kinematic RigidBody
- 인형 접촉: 보이지 않는 곡선 손가락 복합 콜라이더가 실제로 처리

플런저·로커·손가락의 자세는 9절의 고정 길이 조건으로 계산한다. 계산된
로컬 자세를 보이는 기구에 직접 적용하고, 같은 자세를 월드 변환으로 합성해
보이지 않는 손가락 충돌체에 전달한다. 따라서 승강이나 수평 이동 중에도
시각 부품이 분리되지 않으며 인형과의 물리 접촉은 유지된다.
...
| 손가락 충돌체 | Kinematic RigidBody × 3 | 인형과의 마찰·접촉·밀어내기 |
| 승강 와이어 | Rope Joint | 비신축 최대 길이와 진자 운동 |
| 전원 케이블 | 시각 전용 곡선 | 느슨한 나선형 엄빌리컬 표현 |

`getClawPose(q)`가 플런저 위치, 로커 각도, 손가락 힌지와 손가락 각도를
결정하는 단일 기준이다. 렌더링 메시와 충돌체가 각자 위치를 추정해서는
안 된다.
...
| 4 | 보이는 플런저·로커·손가락을 모두 몸통 로컬 계층으로 통합 | 명목상 연결점이 동일 프레임에서 일치 | 현재 방식 |

초기 폐쇄형 구현과 비교해 집게 관련 물리 바디는 9개에서 5개로 줄었다.
전체 장면 기준 계측값은 29개에서 25개로 감소했다. 제거된 바디는 시각
전용 플런저와 로커 링크이며, 인형 접촉용 손가락 충돌체는 유지한다.
