...
- 본체는 Pitch·Yaw·Roll 전축 회전을 허용한다.
- 와이어 연결점을 무게중심 위에 두어 장력이 자연 복원 토크를 만든다.
- Pitch·Roll은 약 `18°`부터 소프트 복원 토크를 적용하고 `25°` 이후 강화한다.
- Yaw에는 별도 복원 토크를 적용하지 않아 스위블 회전을 유지한다.
- 장력 속도 투영 시 회전에 의한 연결점 속도를 분리해 본체 선속도를 보정한다.
...
- 진자 이동과 함께 와이어 끝에서 본체 Pitch·Roll 회전이 발생한다.
- Yaw 회전은 자유롭고 큰 기울기만 소프트 제한된다.
